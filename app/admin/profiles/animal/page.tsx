	...
	useEffect(() => {
		...
		if (savedData) {
			try {
				...
				if (data.vaccinationStatus) setVaccinationStatus(data.vaccinationStatus);
			...
			<div className="max-w-6xl mx-auto p-6">
				...
				<form onSubmit={handleSubmit} className="mt-6 bg-white rounded-xl border p-5 grid gap-4 md:grid-cols-2">
					...
					<div className="flex flex-col gap-1">
						...
						<select value={gender} onChange={e=>setGender(e.target.value)} required className="border rounded-md px-2 py-1 text-sm">
					...
					<div className="flex flex-col gap-1">
						...
						<input value={location} onChange={e=>setLocation(e.target.value)} required className="border rounded-md px-2 py-1 text-sm" placeholder="e.g. UP Mindanao" />
					</div>
					<div className="flex flex-col gap-1">
						<label className="text-xs font-medium text-gray-600">Vaccination Status *</label>
						<select value={vaccinationStatus} onChange={e=>setVaccinationStatus(e.target.value)} required className="border rounded-md px-2 py-1 text-sm">
